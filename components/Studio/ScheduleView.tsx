...
const ScheduleView: React.FC<ScheduleViewProps> = ({ state }) => {
  ...
  return (
    <div className="max-w-4xl mx-auto py-4 sm:py-8 px-2 sm:px-0 animate-fadeIn">
      <h2 className="text-xl sm:text-2xl font-bold mb-6 sm:mb-8 text-center md:text-left">Próximos Encontros</h2>
      ...
      {upcoming.length === 0 ? (
        ...
        <div className="space-y-4">
          {upcoming.map((match) => (
            <div key={match.id} className="glass-effect p-4 sm:p-8 rounded-xl sm:rounded-3xl border border-slate-800 flex items-center justify-center hover:border-emerald-500/30 transition-all group touch-manipulation">
              <div className="flex-1 flex items-center justify-between sm:justify-around gap-2 sm:gap-8 min-w-0">
